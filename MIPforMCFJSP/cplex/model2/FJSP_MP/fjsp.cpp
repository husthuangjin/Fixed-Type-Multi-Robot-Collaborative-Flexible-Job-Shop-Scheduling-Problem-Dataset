...
main(void*) {
    ...
    for (const string& file : files)
    {
        ...
        IntVarMatrix B(env, JobNum);
        for (int i = 0; i < JobNum; i++)
        {
            B[i] = IloNumVarArray(env, procedure_count[i]);
            for (int j = 0; j < procedure_count[i]; j++)
            {
                B[i][j] = IloNumVar(env, 0, IloInfinity, ILOINT);
                string str = "B[" + to_string(i) + "][" + to_string(j) + "]";
                B[i][j].setName(str.c_str());
            }
        }
        ...
        for (int i = 0; i < JobNum; i++)
        {
            for (int j = 0; j < procedure_count[i]-1; j++)
            {
                ...
                model.add(B[i][j] + Sum_Bij1 <= B[i][j+1]);
            }
        }
        //添加约束条件(3-10)和(3-11)
                        ...
                        for (int j1 = 0; j1 < procedure_count[i1]; j1++)
                        {
                            for (int k = 0; k < MacNum; k++)
                            {
                                if (M_table[i][j][k] > 0 && M_table[i1][j1][k]>0)
                                {
                                    ...
                                    model.add(B[i][j] + Sum_Bij2 <= B[i1][j1] + M * (3 - Y[i][j][i1][j1] - Sum_Bij3 - Sum_Bij4));
                                    model.add(B[i1][j1] + Sum_Bij5 <= B[i][j] + M * (2 + Y[i][j][i1][j1] - Sum_Bij3 - Sum_Bij4));
                                }
                            }
                        }
                    }
        ...
        for (int i = 0; i < JobNum; i++)
        {
            ...
            model.add(B[i][procedure_count[i] - 1] + Sum_Bin1 <= C_max);
        }
        //首先添加约束条件(3-13)
        for (int i = 0; i < JobNum; i++)
        {
                        ...
                        if (M_table[i][j][k] > 0) {
                            for (int v = 0; v < muti_machine_num; v++) {
                                if (x_values[i][j][k][v] > 0) {
                                    int start_time = cplex.getValue(B[i][j]);
